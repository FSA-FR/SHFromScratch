...
class WaveFrontError:
    ...
    def generate_phase_map(
        ...
        if self.surface_roughness_nm > 0:
            ...
            # Appliquer un filtre passe-haut pour simuler une rugosité réaliste
        ...
        if self.parallelism_arcsec > 0:
            ...
            tilt_direction = np.pi / 4  # 45 degrés
...
class OpticSpecifications:
    ...
    Attributes:
        ...
        edge_thickness_mm (Optional[float]): Épaisseur au bord pour les lentilles.
        aperture_shape (ApertureShape): Forme de l'aperture (CIRCULAR par défaut).
        width_mm (Optional[float]): Largeur pour les formes non circulaires.
        height_mm (Optional[float]): Hauteur pour les formes non circulaires.
    """
    diameter_mm: float
    thickness_mm: float
...
class OpticalElement(ABC):
    ...
    def get_phase_map(self, grid_x_mm: np.ndarray, grid_y_mm: np.ndarray) -> np.ndarray:
        """FR: Méthode abstraite pour calculer la carte de phase."""
        pass
    
    def get_aperture_mask(self, grid_x_mm: np.ndarray, grid_y_mm: np.ndarray) -> np.ndarray:
        """
        FR: Génère un masque binaire pour l'aperture.
            Supporte: circulaire, rectangulaire, carré, elliptique, hexagonale, octogonale.
        """
    ...
    def get_transmission_map(self, grid_x_mm: np.ndarray, grid_y_mm: np.ndarray, 
        ...
        """FR: Calcule la carte de transmission."""
    ...
    def get_reflection_map(self, grid_x_mm: np.ndarray, grid_y_mm: np.ndarray,
        ...
        """FR: Calcule la carte de réflexion."""
        if self.material is None:
    ...
    def get_full_phase_map(self, grid_x_mm: np.ndarray, grid_y_mm: np.ndarray,
        ...
        """FR: Génère la carte de phase complète (optique + WFE + tilt)."""
    ...
    def apply_to_beam(self, beam: any) -> any:
        """FR: Applique l'optique à un faisceau avec affichage automatique."""
        ...
        if self.display and VISUALIZATION_AVAILABLE:
            ...
            if hasattr(beam, 'intensity') and beam.intensity is not None:
                plot_intensity(beam.intensity, beam.diameter_mm, 
            ...
            if hasattr(beam, 'phase') and beam.phase is not None:
                plot_phase(beam.phase, beam.diameter_mm,
                          title=f"Before {self.name}",
        ...
        if self.display and VISUALIZATION_AVAILABLE:
            if hasattr(new_beam, 'intensity') and new_beam.intensity is not None:
                plot_intensity(new_beam.intensity, new_beam.diameter_mm,
            ...
            if hasattr(new_beam, 'phase') and new_beam.phase is not None:
                plot_phase(new_beam.phase, new_beam.diameter_mm,
                          title=f"After {self.name}",
    ...
    def get_optical_path_difference(self, grid_x_mm: np.ndarray, grid_y_mm: np.ndarray) -> np.ndarray:
        """FR: Calcule la différence de marche optique (OPD)."""
        if self.material is None:
            return np.zeros_like(grid_x_mm)
    ...
    def _get_effective_thickness(self, grid_x_mm: np.ndarray, grid_y_mm: np.ndarray) -> np.ndarray:
        """FR: Méthode abstraite pour l'épaisseur effective."""
        pass
...
class IdealLens(OpticalElement):
    ...
    def __init__(self, name: str, focal_length_mm: float, diameter_mm: float,
        ...
        if specifications is None:
            specifications = OpticSpecifications(
                diameter_mm=diameter_mm, thickness_mm=0.0, material_name=material_name)
...
class SimpleLens(OpticalElement):
    """FR: Lentille simple avec une face courbe."""
    ...
    def __init__(self, name: str, radius_of_curvature_mm: float, diameter_mm: float,
        ...
        if specifications is None:
            specifications = OpticSpecifications(
                diameter_mm=diameter_mm, thickness_mm=thickness_mm, material_name=material_name)
...
class DoubleLens(OpticalElement):
    """FR: Lentille avec deux faces courbes."""
...
class DoubletLens(OpticalElement):
    """FR: Doublet collé (deux lentilles collées)."""
    ...
    def _get_effective_thickness(self, grid_x_mm: np.ndarray, grid_y_mm: np.ndarray) -> np.ndarray:
        return self._get_effective_thickness_1(grid_x_mm, grid_y_mm) + \
               self._get_effective_thickness_2(grid_x_mm, grid_y_mm)
    ...
    def create_achromatic_doublet(focal_length_mm: float, diameter_mm: float,
        ...
        """FR: Crée un doublet achromatique."""
        ...
        return DoubletLens(
            name=f"Achromatic_Doublet_f{focal_length_mm}mm",
            diameter_mm=diameter_mm,
            radius_of_curvature_1_mm=R1,
            radius_of_curvature_2_mm=R2,
            thickness_1_mm=3.0,
            material_1_name=material1_name,
            radius_of_curvature_3_mm=R3,
            radius_of_curvature_4_mm=R4,
            thickness_2_mm=2.0,
            material_2_name=material2_name,
...
class Mirror(OpticalElement):
    ...
    def __init__(self, name: str, diameter_mm: float, mirror_type: MirrorType = MirrorType.FLAT,
        ...
        if specifications is None:
            specifications = OpticSpecifications(
                diameter_mm=diameter_mm, thickness_mm=10.0, material_name=material_name)
...
class Beamsplitter(OpticalElement):
    ...
    def __init__(self, name: str, diameter_mm: float,
        ...
        if specifications is None:
            specifications = OpticSpecifications(
                diameter_mm=diameter_mm, thickness_mm=1.0, material_name=material_name)
    ...
    def get_transmission_map(self, grid_x_mm: np.ndarray, grid_y_mm: np.ndarray,
        ...
        if self.beamsplitter_type == BeamsplitterType.POLARIZING and self.polarization_axis:
            # Simplified polarization handling
...
class Prism(OpticalElement):
    ...
    def __init__(self, name: str, apex_angle_deg: float, base_length_mm: float, height_mm: float,
        ...
        if specifications is None:
            specifications = OpticSpecifications(
                ...
                width_mm=base_length_mm, height_mm=height_mm)
    ...
    def get_deviation_angle(self, wavelength_nm: Optional[float] = None) -> float:
        """FR: Calcule l'angle de déviation en degrés."""
...
class OpticalSystem:
    ...
    def propagate_beam(self, beam: any, initial_position_mm: float = 0.0, use_propagation: bool = True) -> any:
        ...
        if self.display and VISUALIZATION_AVAILABLE:
            ...
            if hasattr(current_beam, 'intensity') and current_beam.intensity is not None:
                plot_intensity(current_beam.intensity, current_beam.diameter_mm,
                              title="Initial Beam", save_path=os.path.join(self.display_dir, "initial_beam.png"))
        ...
        if self.display and VISUALIZATION_AVAILABLE:
            if hasattr(current_beam, 'intensity') and current_beam.intensity is not None:
                plot_intensity(current_beam.intensity, current_beam.diameter_mm,
                              title="Final Beam", save_path=os.path.join(self.display_dir, "final_beam.png"))
...
class TestOptiques:
    ...
    def test_doublet_lens(self):
        ...
        assert phase.shape == grid_x.shape

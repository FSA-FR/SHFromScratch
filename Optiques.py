...
class OpticalElement(ABC):
    ...
    def get_aperture_mask(
        ...
        r_mm = np.sqrt((grid_x_mm - x_center)**2 + (grid_y_mm - y_center)**2)
        
        # Diamètre effectif (en tenant compte du ratio d'ouverture utile)
    ...
    def get_full_phase_map(
        ...
        if include_tilt and (self.tilt_deg[0] != 0 or self.tilt_deg[1] != 0):
            ...
            # Mais on veut φ en nm, donc :
            # φ_nm = (x_mm * 1e-3 * tilt_x_rad + y_mm * 1e-3 * tilt_y_rad) * (wavelength_nm * 1e-9) / (wavelength_nm * 1e-9) * 1e9
            # Simplification : φ_nm = (x_mm * tilt_x_rad + y_mm * tilt_y_rad) * 1e3
            # (car 1e-3 * 1e9 = 1e6, et λ se simplifie)
            # Correction : φ = k * (x*θx + y*θy) où k = 2π/λ
            # φ_nm = φ * λ/(2π) * 1e9 = (2π/λ) * (x*θx + y*θy) * λ/(2π) * 1e9 = (x*θx + y*θy) * 1e9
            # Mais x est en mm, donc x_mm * 1e-3 * θ_rad * 1e9 = x_mm * θ_rad * 1e6
